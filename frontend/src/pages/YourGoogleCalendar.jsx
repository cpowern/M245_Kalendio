...
const YourGoogleCalendar = () => {
  ...
  console.log('Group Code:', groupCode, 'Calendar ID:', calendarId); // Debugging
    ...
    const fetchEvents = async () => {
      ...
      try {
        ...
        if (response.data.success) {
          console.log('Geladene Events:', response.data.events); // Debugging
      ...
      <div className="calendar-container" style={{ marginTop: '20px' }}>
        {events.length > 0 ? (
          ...
          />
            ...
            <p style={{ textAlign: 'center', color: 'red' }}>Keine Termine gefunden. Bitte stelle sicher, dass der Kalender existiert und Events enthält.</p>
        )}
      </div>
    </div>
  );
